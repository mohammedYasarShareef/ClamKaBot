...
function CursorOrb() {
  ...
  return (<>
    <div className="cursor-orb" ref={spotRef} style={{ position: 'fixed', width: 38, height: 38, borderRadius: '50%', mixBlendMode: 'screen', transform: 'translate(-50%,-50%)', pointerEvents: 'none', zIndex: 9998, opacity: 0, transition: 'opacity 0.15s, background 0.2s, box-shadow 0.2s', background: 'radial-gradient(circle,rgba(255,255,255,0.55) 0%,rgba(0,255,255,0.30) 35%,rgba(0,255,255,0.10) 65%,transparent 100%)', boxShadow: '0 0 8px 4px rgba(0,255,255,0.18),0 0 18px 6px rgba(0,255,255,0.08)' }} />
    <div className="cursor-orb" ref={dotRef} style={{ position: 'fixed', width: 5, height: 5, borderRadius: '50%', background: '#00FFFF', pointerEvents: 'none', zIndex: 9999, transform: 'translate(-50%,-50%)', boxShadow: '0 0 6px 2px rgba(0,255,255,0.8)', transition: 'transform 0.05s, background 0.2s, box-shadow 0.2s', opacity: 0 }} />
  </>);
}
...
export default function Dashboard() {
  ...
  return (
    <div style={{ display: 'block', height: '100vh', overflow: 'visible', cursor: 'none', position: 'relative' }}>
      <CursorOrb />
      <style>{`
      ...
      <aside id="cyan-sidebar" style={{ width: 270, minWidth: 240, maxWidth: 300, background: '#00FFFF', display: 'flex', flexDirection: 'column', padding: '32px 0', boxShadow: '4px 0 24px rgba(0,255,255,0.35)', zIndex: 50, flexShrink: 0, cursor: 'default', position: 'fixed', top: 0, left: 0, height: '100vh', transform: sidebarOpen ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform 0.28s cubic-bezier(0.4,0,0.2,1)' }}>
        <div style={{ padding: '0 16px 0 24px', marginBottom: 32 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: '50%', background: '#164e63', border: '2px solid #155e75', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#00E5FF', fontWeight: 800, fontSize: '0.9rem', fontFamily: "'Space Grotesk'", flexShrink: 0 }}>{userName[0]?.toUpperCase()}</div>
            <p className="font-headline" style={{ fontWeight: 700, fontSize: '0.65rem', letterSpacing: '0.12em', textTransform: 'uppercase', color: '#003d41', lineHeight: 1.2, flex: 1 }}>{userName}</p>
            {/* Close arrow button */}
            <button onClick={() => setSidebarOpen(false)} style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(0,60,70,0.15)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#003d41', flexShrink: 0, cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
      ...
      {/* ═══ MAIN WORKSPACE ═══ */}
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', background: '#000', position: 'relative', overflow: 'visible', height: '100vh' }}>
        <Starfield />
        ...
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 16px 0 20px', height: 56, background: 'rgba(10,10,10,0.6)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(255,255,255,0.05)', zIndex: 40, flexShrink: 0 }}>
          {/* ═══ HAMBURGER BUTTON ═══ */}
          <button onClick={() => setSidebarOpen(o => !o)} style={{ width: 44, height: 44, borderRadius: 8, background: 'none', border: 'none', color: '#aaaaaa', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 5, flexShrink: 0, padding: 0, cursor: 'pointer', WebkitTapHighlightColor: 'transparent' }}>
            <span style={{ display: 'block', width: 22, height: 2, background: 'currentColor', borderRadius: 2, transition: 'transform 0.2s, opacity 0.2s', transform: sidebarOpen ? 'translateY(7px) rotate(45deg)' : 'none' }} />
            <span style={{ display: 'block', width: 22, height: 2, background: 'currentColor', borderRadius: 2, transition: 'opacity 0.2s', opacity: sidebarOpen ? 0 : 1 }} />
            <span style={{ display: 'block', width: 22, height: 2, background: 'currentColor', borderRadius: 2, transition: 'transform 0.2s, opacity 0.2s', transform: sidebarOpen ? 'translateY(-7px) rotate(-45deg)' : 'none' }} />
          </button>
