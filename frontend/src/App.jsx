...
function ProtectedRoute({ children }) {
  ...
  useEffect(() => {
    // Silently refresh the token on every protected page load
    // so long-lived sessions never hit an expiry wall
    if (token) {
      refreshToken()
        .then(data => {
        ...
        .catch(() => {
          // Refresh failed (truly expired/invalid) — clear and redirect
